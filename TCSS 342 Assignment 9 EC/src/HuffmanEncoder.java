...
public class HuffmanEncoder {
    ...
    private void encode() {
        ...
        encodedText = new byte[(binaryString.length() / Byte.SIZE) + 1];
        int i;
        for (i = 0; i < binaryString.length() - Byte.SIZE; i += Byte.SIZE) {
            encodedText[i / Byte.SIZE] = (byte) Integer.parseInt(binaryString.substring(i, i + Byte.SIZE), 2);
        }
        if (binaryString.length() % Byte.SIZE != 0) {
            encodedText[(binaryString.length() / Byte.SIZE)] = (byte) Integer.parseInt(binaryString.substring(i) + "0".repeat(binaryString.substring(i).length()));
        }
